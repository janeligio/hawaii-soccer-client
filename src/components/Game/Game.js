...
export default function Game(props) {
    const { id, name, venueId, startTime, endTime, date, playerCount, notes } =
        props;
    ...
    useEffect(() => {
        const endpoint =
            process.env.REACT_APP_SERVER_API + '/venues/' + venueId;
        axios({
            method: 'get',
            url: endpoint,
    ...
    function joinGame(gameId, name, email, playerCount) {
        const endpoint =
            process.env.REACT_APP_SERVER_API + '/games/join?id=' + gameId;
        ...
        <div className="game">
            ...
            >
                <div className="fa-angle-right">
                    {!showContent && (
                        <FaAngleRight size="2em" className="fa-angle" />
                    )}
                    {showContent && (
                        <FaAngleDown size="2em" className="fa-angle" />
                    )}
                </div>
                <div>
                    <h3 className="game-venue-name">
                        <strong>@</strong>
                        {venue.name}
                    </h3>
                    <p className="game-date">
                        <small>
                            {moment(date).format('dddd, MMM D')} from{' '}
            ...
            {showContent && (
                <div className="game-content">
                    ...
                    <p className="game-notes">"{notes}"</p>
        ...
        <div className="join-game-form-container">
            <form className="join-game-form" onSubmit={handleSubmit}>
                ...
                <div className="join-game-form-submit-container">
                    ...
                    <button
                        onClick={closeForm}
                        className="join-game-form-cancel"
                    >
                        Cancel
                    </button>
                </div>
